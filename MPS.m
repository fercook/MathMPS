...
LProduct[A_,B_] :=
    Plus@@MapThread[ConjugateTranspose[#2].#1&,{A,B}];
LProduct[A_,B_,Lm_] :=
    Plus@@MapThread[ConjugateTranspose[#2].Lm.#1&,{A,B}];
RProduct[A_,B_] :=
    Plus@@MapThread[#1.ConjugateTranspose[#2]&,{A,B}];
RProduct[A_,B_,Rm_] :=
    Plus@@MapThread[#1.Rm.ConjugateTranspose[#2]&,{A,B}];
...
MPSProductState[numTensors_,OptionsPattern[]] :=
    Module[ {type = OptionValue[Type],coeffList,spin = OptionValue[Spin],\[Chi] = OptionValue[Bond]},
        Switch[type,
        "Identity",
        coeffList = Table[Table[If[ i==j==1,
                                    {1.0}~Join~Table[0.0,{m,1,spin-1}],
                                    Table[0.0,{m,spin}]
                                ],{k,1,numTensors}],{i,1,\[Chi]},{j,1,\[Chi]}];
        "Decaying",
        coeffList = Table[Table[If[ i==j==1,
                                    Normalize[Table[Exp[-0.5 Log[20](m-1)/spin],{m,1,spin}]],
                                    0
                                ],{k,1,numTensors}],{i,1,\[Chi]},{j,1,\[Chi]}];
        "Random",
...
MPSExpandBond[MPS_,new\[Chi]_] :=
    Module[ {old\[Chi]},
        ...
        If[ old\[Chi]>new\[Chi],
            Print["WARNING: Trying to expand an MPS to a smaller bond dimension"];
            MPS,
...
MPSCanonizeSite[tensor_,matrix_,OptionsPattern[]] :=
    Module[ {sense = OptionValue[Direction],usematrix = OptionValue[UseMatrix],\[Chi]L,\[Chi]R,\[Chi],u,v,t,newTensor},(* Start by multiplying the tensor with the matrix from the previous site *)
        If[ sense=="Right",
            If[ usematrix,
                newTensor = tensor.matrix,
                newTensor = tensor
            ];
...
MPSCanonizationCheck[mps_,OptionsPattern[]] :=
    Module[ {checksite = OptionValue[Site],numTensors,spin,norm},
        numTensors = Length[mps];
        spin = Length[mps[[1]]];
        norm = Sum[
        Total[
...
MPSSiteOperator[tensor_,operator_] :=
    Module[ {},
        Flatten[Transpose[tensor,{3,1,2}].operator.Conjugate[tensor],{{3,1},{2,4}}]
    ];


MPSExpectation[mps_,operator_,site_Integer] :=
    Module[ {L,R,numTensors},
        numTensors = Length[mps];
        R[numTensors+1] = {{1}};
        R[n_] :=
            R[n] = RProduct[mps[[n]],mps[[n]],R[n+1]];
        L[0] = {{1}};
        L[n_] :=
            L[n] = LProduct[mps[[n]],mps[[n]],L[n-1]];
        Chop[
...
MPSExpectation[mps_,operator_,site1_Integer,site2_Integer] :=
    Module[ {},
...
MPSExpectation[mps_,operator1_,site1_Integer,operator2_,site2_Integer] :=
    Module[ {},
        Last[MPSCorrelation[mps,operator1,site1,operator2,site2]]
    ];


MPSCorrelation[mps_,operator1_,site1_Integer,operator2_,site2_Integer] :=
    Module[ {L,R,Lo,numTensors,siteL,siteR,opL,opR,NormalOrder,corr},
        numTensors = Length[mps];
        Which[
        ...
        Lo[n_] :=
            Lo[n] = LProduct[mps[[n]],mps[[n]],Lo[n-1]];
        corr = Table[
...
MPSApproximate[mps_,new\[Chi]_,OptionsPattern[]] :=
    Module[ {sweeps = OptionValue[Sweeps],sweep = 0,tol = OptionValue[Tolerance],L,R,numTensors,defineRight,defineLeft,\[Chi]R,\[Chi]L,new,canon,stillconverging = True,verbose = OptionValue[Verbose],message,success,newmps,overlapBIG,overlap,prevoverlap},
        If[ verbose,
            message = PrintTemporary["Preparing matrices"]
        ];
        ...
        defineRight[temp_] :=
            Module[ {},
                ...
                R[numTensors+1] = {{1}};
                R[n_] :=
                    R[n] = RProduct[mps[[n]],newmps[[n]],R[n+1]];
            ];
        defineLeft[temp_] :=
            Module[ {},
            ClearAll[L];
                L[0] = {{1}};
                L[n_] :=
                    L[n] = LProduct[mps[[n]],newmps[[n]],L[n-1]];
            ];
        ...
        If[ verbose,
            ...
            message = PrintTemporary["Right sweep:"<>ToString[sweep]<>", site:"<>ToString[site]<>", overlap:"<>ToString[1+overlapBIG-2 Re[MPSOverlap[newmps,mps]]]];
            Pause[1]
        ];
        success = False;
        ...
        If[ verbose,
            NotebookDelete[message];
            message = PrintTemporary["Left sweep:"<>ToString[sweep]<>", site:"<>ToString[site]<>", overlap:"<>ToString[1+overlapBIG-2 Re[MPSOverlap[newmps,mps]]]];
            Pause[1]
        ];
        success = False;
        ...
        If[ Abs[(overlap-prevoverlap)/overlap]<tol,
...
MPSSave[MPS_,filename_] :=
    Module[ {numSites,spin},
        numSites = Length[MPS];
        spin = Length[MPS[[1]]];
    ...
    Module[ {energy,prevEnergy = 0,energyList = {},sweeps = OptionValue[Sweeps],sweep = 0,IntRange = OptionValue[InteractionRange],monitorenergy = OptionValue[MonitorEnergy],tol = OptionValue[Tolerance],L,R,fieldL,fieldR,operatorsR,operatorsL,interactionsL,interactionsR,Heff,numTensors,defineRight,defineLeft,\[Chi]R,\[Chi]L,new,canon,stillconverging = True,verbose = OptionValue[Verbose],message,info,success},
        ...
        defineLeft[temp_] :=
            Module[ {},
                ...
                fieldL[n_] :=
                    fieldL[n] = Table[LProduct[mps[[n]],mps[[n]],fieldL[n-1][[\[Alpha]]]]+HMatrix[[\[Alpha],n,n]]LProduct[sigma[\[Alpha]].mps[[n]],mps[[n]],L[n-1]],{\[Alpha],1,3}];
                ...
                interactionsL[n_] :=
                    interactionsL[n] = Table[LProduct[mps[[n]],mps[[n]],interactionsL[n-1][[\[Alpha]]]]+(Reverse[HMatrix[[\[Alpha],Max[n-IntRange,1];;n-1,n]]].(LProduct[sigma[\[Alpha]].mps[[n]],mps[[n]],#]&/@operatorsL[n-1][[\[Alpha]]])),{\[Alpha],1,3}];
            ];
        (* Start from the left, so prepare all right matrices *)
        defineRight[1];
        While[sweep<sweeps&&stillconverging,
        ...
        If[ verbose,
            NotebookDelete[message];
            message = PrintTemporary["Right sweep:"<>ToString[sweep]<>", site:"<>ToString[site]<>", Energy:"<>ToString[energy]]
        ];
        success = False;
        While[!success,
        ...
        If[ monitorenergy,
            energyList = energyList~Join~{{energy,info}}
        ];
        ,{site,1,numTensors}];
        sweep+=0.5;
        (* Sweep to the left clears all right matrices and defines them one by one *)
        ...
        If[ verbose,
            NotebookDelete[message];
            message = PrintTemporary["Left sweep:"<>ToString[sweep]<>", site:"<>ToString[site]<>", Energy:"<>ToString[energy]]
        ];
        success = False;
        While[!success,
        ...
        ,{site,numTensors,1,-1}];
        sweep+=0.5;
        If[ Abs[(energy-prevEnergy)/energy]<tol,
            stillconverging = False,
            prevEnergy = energy
        ];
        ];
...
MPSEffectiveHam[interactionsL_,interactionsR_,fieldL_,fieldR_,operatorsL_,operatorsR_,Hmatrix_] :=
    Module[ {Htemp = 0,\[Chi]L,\[Chi]R,intrangeL,intrangeR,L,R},
(* Preparation of internal matrices and constants *)
        ...
        Htemp+=Sum[Sum[Sum[MPSEffectiveSingleHam[operatorsL[[\[Alpha],x]],operatorsR[[\[Alpha],y]],If[ (x+y-1)<Max[intrangeL,intrangeR],
                                                                                                       Hmatrix[[\[Alpha],intrangeL+1-x,intrangeL+1+y]],
                                                                                                       0
                                                                                                   ]sigma[0]],{\[Alpha],1,3}],{y,1,intrangeR}],{x,1,intrangeL}]; 
        ...
        (* Finally, the interactions between the site and the left and right blocks *)
        Htemp+=Sum[Sum[MPSEffectiveSingleHam[operatorsL[[\[Alpha],x]]Hmatrix[[\[Alpha],intrangeL+1-x,intrangeL+1]],R,sigma[\[Alpha]]],{\[Alpha],1,3}],{x,1,intrangeL}];
...
FindGroundMPSSiteManual[A_,DLeft_,DRight_,hLeft_,hRight_,vLeft_,vRight_,Ham_] :=
    Module[ {H,sol,\[Chi]L,\[Chi]R,spin},
...
EstablishLink[LINK_] :=
    ...
    ClearLink[LINK];
    ClearAll[FindGroundMPSSite,IsLinkActive];
    If[ Length[Links["*arpackformps"]]==0,
        LINK = Install["arpackformps_"<>$SystemID]
    ];
    If[ LINK==$Failed||ForceUseInternalRoutine,
        SetAttributes[FindGroundMPSSite,HoldAll];
        FindGroundMPSSite[A_,DLeft_,DRight_,hLeft_,hRight_,vLeft_,vRight_,Ham_] :=
            FindGroundMPSSiteManual[A,DLeft,DRight,hLeft,hRight,vLeft,vRight,Ham]
    ]
    );
